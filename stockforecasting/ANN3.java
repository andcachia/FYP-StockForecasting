...
public class ANN3{
    ...
    public double[][] train(double[][] input, double[][] target){
        ...
        if (input.length != target.length)
            ...
            hidden2 = new double[hidden_neurons2];
            ...
            WH2 = randomizeWeights(hidden_neurons,hidden_neurons2,-1,1);
            WO = randomizeWeights(hidden_neurons2,target[0].length,-1,1);
            
            //These are initially set to 0
            WH_delta = randomizeWeights(input[0].length,hidden_neurons,0,0);
            WH2_delta = randomizeWeights(hidden_neurons,hidden_neurons2,0,0);
            ...
            while (badEpoch != 0){
                ...
                for (int count=0;count<n_training;count++){
                    ...
                    hidden = forwardPass(input[count], WH);
                    hidden2 = forwardPass(hidden, WH2);
                    OUT[count] = forwardPass(hidden2, WO);
                    ...
                    if (badFact){
                        ...
                        //passes delta values and performs first backward pass
                            backwardPass(WO, WO_delta, hidden2, delta);
                        //computes new delta values for hidden layer, using old delta values
                            delta = newDelta(delta, WO, hidden2);
                            backwardPass(WH2, WH2_delta, hidden, delta);
                            delta = newDelta(delta, WH2, hidden);
    ...
    public double[][] run(double[][] input, double[][] target){
        ...
        for (int i=0; i<input.length; i++){           
            ...
            textArea.append("Input: " + printArray(input[i]));

            //almost same process as before, however only forward pass is done (no backward pass)
            hidden = forwardPass(input[i],WH);
            hidden2 = forwardPass(hidden,WH2);
            OUT[i] = forwardPass(hidden2, WO);

            textArea.append("\tExpected: " + printArray(target[i]));
    ...
    private double[] forwardPass(double[] in_vector, double[][] WH){
    ...
    private void DrawInputs(double[][] input){
        ...
        myframe2.setSize(1200, 650);
        myframe2.setVisible(true);

        //Chart settings and variables
        series2 = new XYSeries("First");
        series3 = new XYSeries("Second");
        dataset2 = new XYSeriesCollection();
        dataset2.addSeries(series2);
        dataset2.addSeries(series3);
        chart2 = createChart(dataset2, "Input Plot", "Position","Value");
        JPanel content = new JPanel(new BorderLayout());
        ...
        for (int i=0;i<input.length;i++){
            series2.add(i,input[i][0]);
            series3.add(i,input[i][2]);
        }
    ...
    public void setHiddenNeurons2(int hidden_neurons2) {
        this.hidden_neurons2 = hidden_neurons2;
    }
